...
#  响应的指定结点是否为空或为预期值
###############################
def node_array_assert?(node_array, json_response)
  flag = true
  node_array.each { |i|
    case i
      when String
        value = JsonPath.on(json_response, i).first
        case value
          when String
            flag &= !value.empty?
        end
        flag &= !(JsonPath.on(json_response, i).first.empty?)
      when Hash
        i.each{|k, v| flag &= (JsonPath.on(json_response, k.to_s).first == v)}
    end
  }
  flag
end
